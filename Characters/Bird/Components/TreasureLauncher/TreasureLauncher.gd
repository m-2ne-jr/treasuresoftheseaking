...
@export var spawn_arc_width: int = 10
...
func spawn_treasure_as_drop(treasure: Treasure) -> void:
	var new_object: TreasureObject = setup_treasure_object(treasure)
...
func launch_treasure_object(treasure_object: TreasureObject, speed: float) -> void:
	var new_angle_min: float = deg_to_rad(-spawn_arc_width * 0.5)
	var new_angle_max: float = deg_to_rad(spawn_arc_width * 0.5)
	var direction: Vector3 = global_basis.y.rotated(
		global_basis.z.normalized(), 
		randf_range(new_angle_min, new_angle_max)
	)
	var force: Vector3 = (direction * speed) + (direction * spawn_force)
...
func drop_object(treasure_object: TreasureObject) -> void:
